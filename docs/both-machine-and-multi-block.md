...
| Persistence while formed | **Multiblock cache only** — members do not persist their item/fluid/heat containers while formed (`TileEntityProgressMultiblockMachine#persists`); the shared brain is saved by the manager's `MultiblockCache`. Otherwise every member would write a full copy of the shared contents into its own NBT (`getHeatCapacitors(null)` etc. resolve to the shared containers while formed) and re-absorb them on the next formation, multiplying the shared heat/fluid by the block count. |
| Absorption on (re)form | **Only members without a cache.** A block that still references the multiblock cache already handed its contents to it (`getCacheID() != null`), so it is skipped on re-formation; blocks with no cache (freshly placed standalone machines) are absorbed and emptied. Heat is absorbed as the difference from the member's own ambient baseline, because the volume-scaled shared capacitor already carries one ambient baseline per block. |
